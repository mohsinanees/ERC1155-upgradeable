...
contract("AWSTERC1155", (accounts) => {
    ...
    describe("mintBatch", async () => {
        it("should mint the tokens in batch when called by the owner", async () => {
            ...
            let uris = ["abc", "xyr", "ajk", "yte", "pop", "juk", "qwe", "ret", "zer"];
            let tx = await contract.mintBatch(accounts[0], ids, uris, values, "0x0");
            const event = tx.logs[0].args;
            for (let i = 0; i < 9; i++) {
                assert.equal(ids[i], event.ids[i].toNumber());
                assert.equal(await contract.uri(ids[i]), uris[i]);
            }
        });
        it("should fail when called by the non-owner", async () => {
            let ids = [2, 3, 4, 5, 6, 7, 8, 9, 10];
